...
void __attribute__((weak)) *snake_malloc(int a) {
  return malloc(a);
}
void __attribute__((weak)) snake_free(void* a) {
  free(a);
} 
...
{
  snake_set_head(p_game, p_game->limits.x/2, p_game->limits.y/2);
  create_new_apple(p_game);
...
int
snake_init(snake_game_t* p_game, int limit_x, int limit_y)
{
  p_game->apple.x = 0;
  p_game->apple.y = 0;
...
void
snake_get_head(snake_game_t* p_game, int *x, int *y)
{
}

void
snake_set_head(snake_game_t* p_game, int x, int y)
{
  struct segment_t *seg_i;
  struct segment_t *next_tail;

  seg_i=p_game->snake.tail;
  while (seg_i && seg_i!=&(p_game->snake.head)) {
  ...
  p_game->snake.head.x = x;
  p_game->snake.head.y = y;
}
